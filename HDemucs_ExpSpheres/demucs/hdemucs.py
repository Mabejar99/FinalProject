...
class HDemucs(nn.Module):
    ...
    def forward(self, mix):
        ...
        if not self.cac:
            # x from [B,S,C,T] to [B,S,T] (selecting channels such that C=S)
            indices = torch.arange(S, device='cuda').view(1, S, 1, 1)
            x = x.gather(2, indices.expand(B, S, 1, length)).squeeze(2)
        else:
            x = x.squeeze(2)
