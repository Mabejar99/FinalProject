...
from .PythonPsychoacoustics.psyacmodel_torch import *
import torch
...
def psyacthresh_torch(ys, fs):
    ...
    return mT.to('cuda')  # the masking threshold in N+1 subbands for the M blocks
...
def percloss(orig, modified, fs):
    ...
    if len(orig.shape) == 2:  # multichannel
        ...
        origys = torch.stft(orig, n_fft=2*N, hop_length=2 *
                        N//2, return_complex=True, normalized=True, window=torch.hann_window(2*N).to('cuda'))
    ...
    modifiedys = torch.stft(
        modified, n_fft=2*N, hop_length=2*N//2, return_complex=True, normalized=True, window=torch.hann_window(2*N).to('cuda'))

    # normalized diff. spectrum:
    # normdiffspec = torch.abs((origys-modifiedys)/mT)
    normdiffspec = smooth_abs_complex((origys-modifiedys)/mT)
...
def smooth_abs_complex(z, epsilon=1e-8):
    return torch.sqrt(z.real ** 2 + z.imag ** 2 + epsilon)
